...
현재 앱은 영어 RP·GA 넓은 표기의 연구 가표본입니다. **2026-09-19 확인: 자동시험 283/283 통과.** 이는 수록된 시험 조건의 결과이며 사전 전수 검수나 학습 효과 검증을 뜻하지 않습니다. 문서에 기록된 9월 16일의 270건과 현재 시험 수는 확인 시점이 다릅니다.
...
- **화면의 모양:** 제2본의 선형 시범 조합을 따른다(`js/display.js`, 표시 프로파일 `linear-trial-v8-vowel-carrier`). 마지막 초성·첫 중성·(단일) 첫 종성만 한 음절로 합성하고, 나머지 자모는 독립 요소로 보인다. 크기는 합성된 핵에 위 강세 규칙을 적용하고, 뒤 모음 요소 50%(제2본 §14 「뒤 요소 50% 크기」), 독립 자음 75%다. 화면에서는 글꼴의 빈 여백을 제외한 실제 획의 외곽을 측정한다(`js/optical.js`). 합성 핵은 실제 획의 세로 높이를 강세별 기준 크기에 맞추며 모양의 종횡비를 유지한다. 합성 핵 밖의 독립 자모는 원래 글꼴 비례를 유지한다. 뒤 모음 요소(아래아 포함)는 기본 글꼴 크기의 50%, 독립 자음은 75%로 표시하며, 합성 핵의 높이에 맞춰 추가 확대하지 않는다. 모든 비율은 사용자가 정한 기본 글자 크기에 대한 절대 비율이며, 강세 배율을 중첩하지 않는다 — `부우ㅅㅌ`은 부(100) 우(50) ㅅㅌ(75). 종성 /s/와 /t+s/는 언제나 독립 요소다 — 제2본 §9.4가 금지한 `팻`·`캧` 같은 모양이 화면에 나오지 않는다(`패ㅅ`·`캐ㅊ`). 2026-09-17 저자 결정으로 장모음·복모음의 뒤 모음 요소에 읽기용 무음 초성 ㅇ을 붙인다. 옛한글 모음도 ㅇ과 함께 하나의 표시 요소로 조합한다. 이 ㅇ은 새 음소나 음절을 뜻하지 않는다. 이 규칙은 화면만 바꾸며 복사·저장되는 NFD 직렬화는 그대로다.
...
| 장모음·복모음 | 저장은 같은 핵의 V 배열. 읽기용 표시는 뒤 요소에 무음 ㅇ을 붙임 (부우ㅅㅌ) |
